...
# Open MIDI file
midi = mido.MidiFile('MidiSound.mid')

# Get the number of tracks in the MIDI file
num_tracks = midi.numTracks()

# Loop through each track and print its contents
for track_idx in range(num_tracks):
    midi.trackIndex(track_idx)
    print(f"Track {track_idx + 1}:")
    
    # Get the events in the track
    events = midi.events()
    
    # Loop through each event and print its contents
    for event in events:
        print(event)
