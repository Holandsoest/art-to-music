...
class Square(Shape):
    def __init__(self, box:loc.Box, fill_color:str, outline_color:str, rotation_rad=0.0):
        ...
        # store the outline in a list
        self.outline_coordinates = calculate_shape_arms_(center_pos=self.center_pos, traces=4, length_traces=self.radius, rotation=self.rotation_rad + math.pi/4)
class Heart(Shape):
...
class Star(Shape):
    def __init__(self, box:loc.Box, fill_color:str, outline_color:str, rotation_rad=0.0, depth_percentage=50):
        ...
        outer_points = calculate_shape_arms_(center_pos=self.center_pos, traces=5, length_traces=self.radius, rotation=self.rotation_rad + math.pi/2)
        inner_points = calculate_shape_arms_(center_pos=self.center_pos, traces=5, length_traces=self.radius / 100 * depth_percentage,
                                             rotation=self.rotation_rad + (math.pi / 5.0) + math.pi/2)
